...
To create a new account group, enable **Developer Mode** and head to :menuselection:`Accounting
dashboard --> Configuration --> Accounting: Account Groups`. Here, create a new group and enter the
**name, code prefix, and company** to which that group account should be available. Note that you
must enter the same code prefix in both :guilabel:`From` and :guilabel:`to` fields.

.. image:: chart_of_accounts/account-group.png
   :align: center
